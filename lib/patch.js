...
const MAIN_PATCH_ADDITIONS = `
  ...
  async function pmResolveCwd(event) {
    try {
      var cwd = await event.sender.executeJavaScript('(async function(){try{var u=window.api&&window.api.getDefaultServerUrl;if(!u)return null;var base=await u();if(!base)return null;var r=await fetch(base+"/project/current");if(!r.ok)return null;var d=await r.json();return d&&(d.directory||d.path||d.cwd)||null;}catch(e){return null;}})()');
  ...
  function pmGetProjectDir() {
    ...
    if (cfg.remote) {
      ...
      return join(pmGetSyncRepoDir(), "projects", "_local");
    }
    return join(
      process.env.XDG_CONFIG_HOME || join(homedir(), ".config"),
