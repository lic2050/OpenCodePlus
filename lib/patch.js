...
const MAIN_PATCH_ADDITIONS = `
  ...
  function pmGitRun(args, cwd) {
    try {
      pmGitLastError = '';
      var parsed = pmGitParseArgs(args);
      var r = spawnSync('git', parsed, { encoding: "utf8", timeout: 30000, cwd: cwd || pmGetSyncRepoDir(), env: pmGitEnv });
  ...
  ipcMain.handle("pm-sync-init", async (_event, remoteUrl, strategy) => {
    try {
      ...
      pmGitRun("config user.name OpenCodePlus", dir);
      pmGitRun("config user.email opencodeplus@local", dir);
      pmGitRun("add -A", dir);
  ...
  ipcMain.handle("pm-sync-push", async () => {
    try {
      ...
      var _dbg = [];
      _dbg.push("dir=" + dir);
      _dbg.push("exists=" + existsSync(dir));
      var status = pmGitRun("status --porcelain", dir);
      _dbg.push("status=" + JSON.stringify(status));
      if (status) {
        var addResult = pmGitRun("add -A", dir);
        _dbg.push("add=" + JSON.stringify(addResult));
        pmGitRun("config user.name OpenCodePlus", dir);
        pmGitRun("config user.email opencodeplus@local", dir);
        var commitResult = pmGitRun('commit -m "OpenCodePlus: sync ' + new Date().toISOString().slice(0,19) + '"', dir);
        _dbg.push("commit=" + JSON.stringify(commitResult));
        _dbg.push("commitErr=" + pmGitLastError);
      } else {
        _dbg.push("no changes, skipping add/commit");
      }
      if (pmGitRun("rev-parse HEAD", dir) === null) {
        pmGitRun("add -A", dir);
        pmGitRun('commit --allow-empty -m "OpenCodePlus: initial"', dir);
        _dbg.push("created initial commit");
      }
      var branch = pmGitRun("branch --show-current", dir) || "main";
      _dbg.push("branch=" + branch);
      const result = pmGitRun("push -u origin " + branch, dir);
      _dbg.push("push=" + JSON.stringify(result));
      _dbg.push("gitError=" + pmGitLastError);
      const cfg = pmReadSyncConfig();
      cfg.lastSync = new Date().toISOString();
      pmWriteSyncConfig(cfg);
      return { ok: result !== null, error: result === null ? pmGitLastError : undefined, debug: _dbg.join(" | ") };
  ...
  ipcMain.handle("pm-sync-pull", async () => {
    try {
      ...
      if (!headExists) {
        ...
        if (pmGitRun("pull --rebase --autostash origin", dir) === null) return { ok: false, error: pmGitLastError || "pull failed" };
      }
      const cfg = pmReadSyncConfig();
      cfg.lastSync = new Date().toISOString();
      pmWriteSyncConfig(cfg);
...
function hasPatch(content) {
  return content.includes(PATCH_VERSION);
}
...
function install() {
  ...
  if (!preloadJs.includes(PATCH_VERSION)) {
