...
function resolveGitPath() {
  ...
  try {
    _gitPath = require("child_process").execFileSync("where", ["git"], { encoding: "utf8", timeout: 5000 }).trim().split("\n")[0];
    if (_gitPath && fs.existsSync(_gitPath)) return _gitPath;
  } catch(e) {}
  var candidates = [
  ...
  for (var i = 0; i < candidates.length; i++) {
    if (fs.existsSync(candidates[i])) { _gitPath = candidates[i]; return _gitPath; }
  }
  _gitPath = "git";
  return _gitPath;
}
...
const MAIN_PATCH_ADDITIONS = `
  ...
  try {
  var _pmExecFile = require("child_process").execFileSync;
  var _pmExistsSync = require("fs").existsSync;
  ...
  ipcMain.handle("pm-get-cwd", () => { try { return process.cwd(); } catch (e) { return ""; } });
  ...
  function pmGitRun(args, cwd) {
    try {
      var gitCmd = "git";
      try {
        var whereOut = _pmExecFile("where", ["git"], { encoding: "utf8", timeout: 5000 }).trim().split("\\n")[0];
        if (whereOut && _pmExistsSync(whereOut)) gitCmd = whereOut;
      } catch(e) {
        var candidates = [
          "C:\\\\Program Files\\\\Git\\\\cmd\\\\git.exe",
          "C:\\\\Program Files (x86)\\\\Git\\\\cmd\\\\git.exe",
        ...
        for (var i = 0; i < candidates.length; i++) {
          if (_pmExistsSync(candidates[i])) { gitCmd = candidates[i]; break; }
        }
      }
      var argList = args.split(" ");
      return _pmExecFile(gitCmd, argList, { encoding: "utf8", timeout: 30000, cwd: cwd || pmSyncDir() }).trim();
    } catch (e) { console.error("[PM] git error:", e.message); return null; }
  }
  ...
  ipcMain.handle("pm-sync-init", async (_event, remoteUrl, strategy) => {
    try {
      ...
      if (!isGitDir) {
        ...
        if (initResult === null) return { ok: false, error: "git init failed - is git installed?" };
        pmGitRun("checkout -b main", dir);
      }
      pmGitRun("remote remove origin", dir);
      pmGitRun("remote add origin " + remoteUrl, dir);
      pmGitRun("fetch origin", dir);
      ...
      pmGitRun("add -A", dir);
      pmGitRun('commit -m "OpenCodePlus: initial sync"', dir);
  ...
  ipcMain.handle("pm-sync-push", async () => {
    try {
      ...
      const result = pmGitRun("push", dir);
      const cfg = pmReadSyncConfig();
      cfg.lastSync = new Date().toISOString();
      pmWriteSyncConfig(cfg);
      return { ok: result !== null };
    } catch (e) { console.error("[PM] pm-sync-push error:", e); return { ok: false }; }
  });

  ipcMain.handle("pm-sync-pull", async () => {
    try {
      const dir = pmSyncDir();
      const result = pmGitRun("pull --rebase", dir);
      const cfg = pmReadSyncConfig();
      cfg.lastSync = new Date().toISOString();
      pmWriteSyncConfig(cfg);
      return { ok: result !== null };
    } catch (e) { console.error("[PM] pm-sync-pull error:", e); return { ok: false }; }
  });
  ...
  ipcMain.handle("pm-sync-resolve", async (_event, strategy) => {
    try {
      ...
      if (strategy === "cloud") {
        ...
        pmGitRun("add -A", dir);
        pmGitRun('commit -m "OpenCodePlus: resolve to local"', dir);
        pmGitRun("push -u origin main", dir);
      }
      const cfg = pmReadSyncConfig();
      cfg.lastSync = new Date().toISOString();
