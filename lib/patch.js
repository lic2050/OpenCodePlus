...
const MAIN_PATCH_ADDITIONS = `
  ...
  ipcMain.handle("pm-get-cwd", () => { try { return _pmCachedCwd; } catch (e) { return ""; } });
  ...
  function pmGetProjectKey() {
    try {
      var url = pmGitRun("remote get-url origin", _pmCachedCwd);
      if (!url) return null;
      url = url.replace(/\\.git$/, '')
  ...
  ipcMain.handle("pm-get-project-key", () => { try { return pmGetProjectKey(); } catch (e) { return null; } });
